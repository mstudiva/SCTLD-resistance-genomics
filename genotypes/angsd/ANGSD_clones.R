...
cloneDendA = ggplot() +
  ...
  geom_hline(yintercept = 0.139, color = "red", lty = 5, size = 0.75) + # creating a dashed line to indicate a clonal distance threshold
...
mlg.filter(snpclone_rad, threads = 1L) <- 0.174 # Applies the filter based on the dissimilarity matrix
# mlg.filter(snpclone_rad, threads = 1L) <- 0.139 # Applies the filter based on the dissimilarity matrix
